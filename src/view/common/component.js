$.template = (function() {
    return {
        compile: function(template, data, attr, slots, key) {
            ...
            functionBody = functionBody.replace(/\$\$/, "@@").replace(/\$/g, key).replace(/@@/, "$").replace(/'/g, "\"").replace(/{%(.*?)%}/g, function(match, p) {
                return "';" + p + "html+='";
                    ...
                    (function(currentElement, currentComponent, key) {
                        ...
                        for (var n in slotElements) { //把html中的信息（属性）读取出来
                            if (slotElements[n].nodeType === 1) {
                                ...
                                slot.template = slotElements[n].innerHTML; //{{value}}
                                slots.push(slot);
                            }
                        }
...
$.component(function() {
    return {
        ...
        template: "<div>\
                        ...
                        <input type='file' onChange='$.chooserFile(this.value)'/>\
                    </div>",
        state: {
            value: "",
        },
        controller: function() {},
        chooserFile: function(value) {
            this.setState({
                value: value
            })
        }
...
$.component(function() {
    return {
        ...
        state: {
            items: [{ title: "haha" }, { title: "haha" }, { title: "haha" },
